...
def main(): 
    html_temp="""
     <div style = "background-color:green;padding:16px; border-radius: 15px;">
     <h2 style="color:black;text-align:center;"> Kenyan Used Car Price Predictor</h2>
     </div>
    """
    st.markdown(html_temp,unsafe_allow_html=True)
   
    st.markdown("##### Planning To Sell or Buy a car !?\n##### So let's get the price..")
    
    st.write('')
    st.write('')
    p1 = st.number_input('What is the current ex-showroom price of the car ?  (/100,000)',2.5,25.0,step=1.0) 
    ...
    try: 
        if st.button('Predict'):
            ...
            if prediction>0:
                st.balloons()
                st.success('You can sell the car for {:.2f} 100,000 Ksh'.format(prediction[0]))
            else:
